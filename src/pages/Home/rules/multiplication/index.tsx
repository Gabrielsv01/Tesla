...
import {calcMult} from 'shared/utils/calcLevel';
...
const MultiplicationRules: React.FC = () => {
  const [result, setResult] = useState<{
    ...
    primitive: number;
    transporte: number;
    interaçãoNíveis: number;
    cruzamentos: number;
  }>();
  const [firstValue, setFirstValue] = useState(0);
  const [secondValue, setSecondValue] = useState(0);

  const handleCalcMult = (a: number, b: number) => {
    setResult(calcMult(a, b));
  };
    ...
    <S.Card>
      ...
      <S.CardWrapper disableMaxHeight>
        ...
        <S.Description>
          ...
          <br />
          <br />
          O primitivo do resultado é o produto dos primitivos de A e B, reduzido
          a um único dígito (Prova dos Nove).
          <br />
          <br />
          <b>P(C) = P(P(A) * P(B))</b>
        </S.Description>
        ...
        <S.Description>
          ...
          <ul>
            ...
            <br />
            <li>
              <b>1. Interação Níveis:</b> <code>9 * (16 * 12) =</code>{' '}
              <b>1728</b>
            </li>
            <li>
            ...
            <li>
              ...
              <br />
              Como 48 não divide por 9, utiliza apenas a parte inteira, no caso
              5: <code>⌊48 / 9⌋ =</code> <b>5</b>
            </li>
            <br />
            <li>
        ...
        <S.Description>
          <S.SubTitle>
            Exemplo de Transporte (Caso Simples): 125 * 13
          </S.SubTitle>
          <br />
          <ul>
            <li>
            ...
            <br />
            <li>
              <b>Transporte:</b> Produto <code>32</code>.
              <code> ⌊32 / 9⌋ = </code> <b>3</b>
            </li>
            <li>
        ...
        <div
          ...
          <S.Input
            ...
            onChange={value => {
              ...
              handleCalcMult(parsedValue, secondValue);
            }}
          />
          <S.Input
            inputMode="numeric"
            ...
            onChange={value => {
              ...
              handleCalcMult(firstValue, parsedValue);
            }}
          />
        </div>
        <S.CardWrapper>
          <div>Level: {result?.level.toString()}</div>
          <div>Primitivo: {result?.primitive.toString()}</div>
          <div>Transporte: {result?.transporte.toString()}</div>
          <div>Interação níveis: {result?.interaçãoNíveis.toString()}</div>
          <div>Cruzamentos: {result?.cruzamentos.toString()}</div>
          <div>Value: {result?.value.toString()}</div>
        </S.CardWrapper>
      </S.CardWrapper>
    </S.Card>
  );
